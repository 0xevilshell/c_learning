...
int main() {
    ...
    if (*p == NULL) { /*p — это параметр функции, и он не может быть NULL, если функция уже была вызвана корректно. Проверять нужно не сам указатель ptr, а результат выделения памяти с помощью malloc(), т.е. *p */
        perror("Не удалось выделить память");
        return 1;
    }

    // Присваивание значения 10 в выделенную память
    *p = 10;
